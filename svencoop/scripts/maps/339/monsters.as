...
mixin class monster_qgeneric
{
	...
	float m_flSearchTime     = 0.0f;
	float m_flPauseTime      = 0.0f;
	float m_flAttackFinished = 0.0f;
	...
	float m_flMonsterSpeed  = 0.0f;
	float m_flMoveDistance  = 0.0f; // last move distance. Used for strafe
	bool  m_fLeftY          = false;

	float m_flSightTime     = 0.0f;
	int   m_iRefireCount    = 0;

	float        m_flEnemyYaw     = 0.0f;
	...
	bool MonsterCheckAttack()
	{
		...
		if ( m_iEnemyRange == RANGE_MELEE && MonsterHasMeleeAttack() )
		{
			MonsterMeleeAttack();
			return true;
		}
	...
	void MonsterThink()
	{
		...
		// NOTE: keep a constant interval to make sure events work as predicted
		float flInterval = self.StudioFrameAdvance( 0.099f ); // animate
	...
	// --------- FIXED: null-safe TakeDamage for all Quake-style monsters ---------
	                ...
	                float flDamage, int bitsDamageType )
	{
		...
		CBaseEntity@ pAttacker  = (pevAttacker  is null) ? null : g_EntityFuncs.Instance( pevAttacker );
		CBaseEntity@ pInflictor = (pevInflictor is null) ? null : g_EntityFuncs.Instance( pevInflictor );

		float dmg_save = 0.0f;
		float dmg_take = Math.Ceil( flDamage - dmg_save );

		// figure momentum add (only if we have a valid inflictor)
		if ( pInflictor !is null && !pInflictor.IsInWorld() )
		{
			Vector dir = ( self.pev.origin - pInflictor.Center() ).Normalize();
			self.pev.velocity = self.pev.velocity + dir * flDamage * 8.0f;
		}
		...
		// Retarget to whoever hurt us, if valid
		if ( pAttacker !is null && self !is pAttacker && pAttacker !is pEnemy )
		{
			// Don't infight with same type unless qarmy
			if ( self.GetClassname() != pAttacker.GetClassname() || self.GetClassname() == "monster_qarmy" )
			{
				if ( m_hEnemy && pEnemy !is null && pEnemy.GetClassname() == "player" )
		...
		if ( MonsterHasPain() )
		{
			...
			self.pev.pain_finished = g_Engine.time + 5.0f;
		}
	...
	void Killed( entvars_t@ pevAttacker, int iGib )
	{
		...
		MonsterDeathUse( m_hEnemy, self, USE_TOGGLE, 0.0f );
	}
	...
	bool FindTarget()
	{
		...
		if ( pTarget.GetClassname() != "player" )
		{
			...
			{
				m_flPauseTime = g_Engine.time + 3.0f;
	...
	bool FacingIdeal()
	{
		...
		return (flDelta < 45.0f || flDelta > 315.0f);
	}
	...
	void FoundTarget()
	{
		...
		if ( pEnemy !is null && pEnemy.GetClassname() == "player" )
		{
			...
			// chain alert
			q1_AlertMonsters( pEnemy, self.pev.origin, 512.0f );
		}
	...
	bool MoveStep( const Vector &in vecMove, bool relink )
	{
		...
		float  stepSize = 18.0f; // default stepsize in Sven

		neworg.z += stepSize - 0.5f;
		Vector end = neworg;
		end.z -= stepSize * 2.0f - 0.5f;
	...
	// StepDirection already uses engine WalkMove directly
	bool StepDirection( float flYaw, float flDist )
	{
		float oldIdeal = self.pev.ideal_yaw;
		float oldYaw   = self.pev.angles.y;
		Vector oldOrg  = self.pev.origin;

		self.pev.ideal_yaw = flYaw;
		g_EngineFuncs.ChangeYaw( self.edict() );

		if ( g_EngineFuncs.WalkMove( self.edict(), self.pev.angles.y, flDist, WALKMOVE_NORMAL ) == 0 )
		{
			self.pev.ideal_yaw = oldIdeal;
			self.pev.angles.y  = oldYaw;
			g_EntityFuncs.SetOrigin( self, oldOrg );
			return false;
		}
	...
	{
		float olddir     = Math.AngleMod( int(self.pev.ideal_yaw / 45) * 45.0f );
		...
		// compute |x| and |y| manually (no fabs)
		float ax = (delta.x >= 0.0f) ? delta.x : -delta.x;
		float ay = (delta.y >= 0.0f) ? delta.y : -delta.y;
		...
		// other directions
		bool useSwap = ( ((Math.RandomLong( 0, 2 ) & 3) & 1) != 0 || ay > ax );
	...
	// === Movement helpers using engine WalkMove directly ===

	void AI_Forward( float flDist )
	{
		g_EngineFuncs.WalkMove( self.edict(), self.pev.angles.y, flDist, WALKMOVE_NORMAL );
	}

	void AI_Backward( float flDist )
	{
		g_EngineFuncs.WalkMove( self.edict(), self.pev.angles.y + 180.0f, flDist, WALKMOVE_NORMAL );
	}

	void AI_Pain( float flDist )
	{
		AI_Backward( flDist );
	}

	void AI_PainForward( float flDist )
	{
		g_EngineFuncs.WalkMove( self.edict(), self.pev.ideal_yaw, flDist, WALKMOVE_NORMAL );
	}
	...
	void AI_Run( float flDist )
	{
		...
		if ( !m_hEnemy || pEnemy.pev.health <= 0 )
		{
			...
			else
			{
				if ( m_hMoveTarget )
				{
					// stay over defeated player a few seconds then continue patrol
		...
		if ( m_fEnemyVisible )
			m_flSearchTime = g_Engine.time + 1.0f;
		...
		// HACK: makes AI turn better when chasing
		if ( m_fEnemyVisible )
			AI_Face();
	}
	...
	void AI_Run_Slide()
	{
		...
		float ofs = m_fLeftY ? 90.0f : -90.0f;

		if ( g_EngineFuncs.WalkMove( self.edict(), self.pev.ideal_yaw + ofs, m_flMoveDistance, WALKMOVE_NORMAL ) != 0 )
			return;

		m_fLeftY = !m_fLeftY;

		g_EngineFuncs.WalkMove( self.edict(), self.pev.ideal_yaw - ofs, m_flMoveDistance, WALKMOVE_NORMAL );
	}
	...
	void AI_Charge_Side()
	{
		...
		g_EngineFuncs.WalkMove( self.edict(), heading, 20.0f, WALKMOVE_NORMAL );
	}
	...
	void AI_Melee()
	{
		...
		float ldmg = ( Math.RandomFloat( 0, 3 ) +
		               Math.RandomFloat( 0, 3 ) +
		               Math.RandomFloat( 0, 3 ) );
	...
	void FlyMonsterInitThink()
	{
		...
		// move back to spawn (Sven drops to floor)
	...
	void FlyMonsterInit()
	{
		...
		self.pev.nextthink += Math.RandomFloat( 0.1f, 0.4f );
...
{
	// (left empty; hook if you want Quake-style meat spray later)
}
